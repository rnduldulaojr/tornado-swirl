...
def set_response_code(fsm_obj):
    global _last_response_code_detected
    print("Setting ", _last_response_code_detected)
    fsm_obj._cur_code = _last_response_code_detected



def process_response(fsm_obj):
    lines = fsm_obj._buffer.splitlines()
    cleaned_lines = _clean_lines(lines)
    for line in cleaned_lines:
        matcher = PARAM_MATCHER.match(line)
        if matcher:
            ...
            fsm_obj.path_spec.responses[fsm_obj._cur_code] = param
    fsm_obj._buffer = ""
    


def process_errors(fsm_obj):
    lines = fsm_obj._buffer.splitlines()
    cleaned_lines = _clean_lines(lines)
    ...
    for line in cleaned_lines:
        ...
        if matcher:
            param = Param(name=matcher.group('code'),dtype=None, description=matcher.group('description'), ptype='response')
...
def is_generic_line(line):
    ...
    if is_response_line(line):
        return False
    if is_error_responses_line(line):
        return False
...
def is_error_responses_line(line):
    return line.strip().lower() in ("errors:", "error:", "error response:", "error responses:", "default:", "default response:")

_last_response_code_detected = None


def is_response_line(line):
    global _last_response_code_detected
    matcher = RESPONSE_MATCHER.match(line.strip())
    if matcher is not None:
        code = matcher.group('code') if matcher.group('code') else '200'
        print("Code detected: ",code)
        _last_response_code_detected = code
        return code
    return False
...
FSM_MAP = (
    ...
    {'src': S_BLANK, 'dst': S_PATH_PARAMS,
        'condition': is_path_params_line, 'callback': transition_blank, },
    {'src': S_PATH_PARAMS, 'dst': S_PATH_PARAMS,
        'condition': is_generic_line, 'callback': transition_buffer, },
    {'src': S_PATH_PARAMS, 'dst': S_BLANK,
        'condition': is_blank_line, 'callback': transition_path_params, },
    {'src': S_BLANK, 'dst': S_QUERY_PARAMS,
        'condition': is_query_params_line, 'callback': transition_blank, },
    {'src': S_QUERY_PARAMS, 'dst': S_QUERY_PARAMS,
        'condition': is_generic_line, 'callback': transition_buffer, },
    {'src': S_QUERY_PARAMS, 'dst': S_BLANK,
        'condition': is_blank_line, 'callback': transition_query_params, },
    {'src': S_BLANK, 'dst': S_REQUEST_BODY,
        'condition': is_request_body_line, 'callback': transition_blank, },
    {'src': S_REQUEST_BODY, 'dst': S_REQUEST_BODY,
        'condition': is_generic_line, 'callback': transition_buffer, },
    {'src': S_REQUEST_BODY, 'dst': S_BLANK,
        'condition': is_blank_line, 'callback': transition_request_body, },
    {'src': S_BLANK, 'dst': S_HEADER, 'condition': is_header_line,
        'callback': transition_blank, },
    {'src': S_HEADER, 'dst': S_HEADER, 'condition': is_generic_line,
        'callback': transition_buffer, },
    {'src': S_HEADER, 'dst': S_BLANK, 'condition': is_blank_line,
        'callback': transition_header, },
    {'src': S_BLANK, 'dst': S_COOKIE, 'condition': is_cookie_line,
        'callback': transition_blank, },
    {'src': S_COOKIE, 'dst': S_COOKIE, 'condition': is_generic_line,
        'callback': transition_buffer, },
    {'src': S_COOKIE, 'dst': S_BLANK, 'condition': is_blank_line,
        'callback': transition_cookie, },
    {'src': S_BLANK, 'dst': S_RESPONSES, 'condition': is_response_line,
        'callback': set_response_code, },
    {'src': S_RESPONSES, 'dst': S_RESPONSES,
        'condition': is_generic_line, 'callback': transition_buffer, },
    {'src': S_RESPONSES, 'dst': S_BLANK,
        'condition': is_blank_line, 'callback': process_response},
    {'src': S_BLANK, 'dst': S_ERROR_RESPONSES,
        'condition': is_error_responses_line, 'callback': transition_blank,},
    {'src': S_ERROR_RESPONSES, 'dst': S_ERROR_RESPONSES, 
        'condition': is_generic_line, 'callback': transition_buffer, },
    {'src': S_ERROR_RESPONSES, 'dst': S_BLANK,
        'condition': is_blank_line, 'callback': process_errors},
